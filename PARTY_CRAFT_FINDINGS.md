...
| The Storyteller's chaos is the point, not a bug **[Medway, primary]** | Behind the Curtain #1 directly poses the objection "If the Storyteller can lie to players about their info, isn't it all just guesswork? Why is this fun?" Medway's answer: the Storyteller's joy is crafting the right level of challenge; the evil player's joy is crafting the perfect lie; the good player's joy is "to cut through the chaos, to find the necessary truth amidst a sea of possibilities, to separate fact from fiction, and to work together to build a team that agrees with you." **DIVERGENCE — do not carry forward as-is:** Clocktower's "team" framing describes a cooperative good-vs-evil structure. Choose Your Mystery's accusation phase is individually competitive — `POST /games/{id}/accuse` is first-correct-guess-wins-alone, not a team outcome, and wrong guesses aren't coordinated with teammates. Reuse the *triangulating partial/contradictory info* insight (that part transfers directly), but adapt "build a team that agrees with you" to each player triangulating their *own* read of the shared clue pool — do not generate resolution/accusation text that frames players as winning or verifying truth together. | **Accusation/Reveal Phase**, **75% Sharing Mechanic** — direct validation of the 75%-sharing design intent: the payoff isn't players ending up with perfect information, it's the social process of triangulating partial/contradictory info into a personal read of the truth. Confirms that imperfect propagation is a feature, not something to "fix" toward full transparency — but see the divergence note in the Insight column before this reaches any accusation/resolution-phase generation prompt. | [Behind the Curtain #1: Total Chaos? Sort Of...](https://bloodontheclocktower.com/news/behind-the-curtain-1-total-chaos-sort-of) |
...
1. **Win Condition Design as a live tension** — Jackbox's "winning is secondary" ethos vs.
   ...
   **Confirmed structural, not just tonal (verified against real code):** Clocktower's
   design goes further than Jackbox's — it's built around a *team* consensus outcome (good
   team wins together), which actively conflicts with `POST /games/{id}/accuse`'s real
   behavior (first correct guess wins alone, wrong guesses aren't team-coordinated). See the
   divergence note added directly to the Medway "Storyteller's chaos" row above — that's the
   row most likely to get pulled into a future Accusation/Reveal-Phase-tagged generation call
   (e.g. the end-game resolution scene), so the caveat lives in the row's own Insight text,
   not just here, to travel with the content wherever it's retrieved.
