...
Two entry points, each backed by a coherence.engine.RuleSet subclass (see
MysteryPartsRuleSet / MysteryRuleSet below) — check_parts()/check_mystery()
are thin wrappers kept for backward compatibility with existing callers:
...
from coherence.engine import BLOCKING, WARNING, INFO, Issue, RuleSet
from coherence.engine import CoherenceReport as _EngineCoherenceReport


# ─── Result dataclasses ──────────────────────────────────────────────────────
# Issue and the severity constants now come from coherence.engine — the
# shared vocabulary across every project's RuleSet. WitnessGap stays local:
# it's a CYM-specific shape (a character + which interrogation anchors it's
# missing), not something the generic engine needs to know about.
...
@dataclass
class CoherenceReport(_EngineCoherenceReport):
    """
    CYM's coherence report. Extends the engine's base CoherenceReport
    (passed / issues / metadata) with CYM's categorized check families —
    `issues` is populated in __post_init__ as the union of p1_issues +
    scene_issues + part_issues (witness_gaps isn't an Issue subtype, so it's
    excluded, matching the original blocking/warning semantics: witness gaps
    degrade gameplay but never block).
    """
    ...
    def __post_init__(self) -> None:
        self.issues = self.p1_issues + self.scene_issues + self.part_issues
...
class MysteryPartsRuleSet(RuleSet):
    """
    Validates a list of sampled MysteryPart objects BEFORE calling Claude.
    ...
    run(parts) returns a CoherenceReport. If passed=False, at least one
    BLOCKING issue was found; the caller should re-sample the flagged
    part_types before proceeding.
    """
    ...
    @staticmethod
    def _check(parts) -> CoherenceReport:
        issues: List[Issue] = []
...
def check_parts(parts) -> CoherenceReport:
    """Thin wrapper kept for backward compatibility — see MysteryPartsRuleSet."""
    return MysteryPartsRuleSet().run(parts)
...
class MysteryRuleSet(RuleSet):
    """
    Validates a fully generated mystery dict against three check families:
    ...
    run(mystery) returns a CoherenceReport. BLOCKING issues mean the mystery
    should not be presented to players without a repair pass.
    """
    ...
    @staticmethod
    def _check(mystery: dict) -> CoherenceReport:
...
def check_mystery(mystery: dict) -> CoherenceReport:
    """Thin wrapper kept for backward compatibility — see MysteryRuleSet."""
    return MysteryRuleSet().run(mystery)
