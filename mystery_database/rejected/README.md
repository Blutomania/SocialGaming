...
| `the_last_night_of_delacroix_&_sons` | passed, 0 blocking, 1 warning | **Two new holes, both in the rules rather than the mystery.** E9 narrowed to *[Celestine Vautrain, Émile Delacroix]* — two names, but Émile is the **victim**, so it narrowed to one living suspect and solved the case outright. The rule counted list entries, not suspects, so a dead man widened a narrowing. And its prose named the culprit as *"the inventory entry **Celestine** herself made"* — first name only, which the leak check missed because it compared against the full name. Both rules are now tighter: narrowing counts suspects, and the prose check matches name words. Separately, Nadège Fontenot was cleared by one finding — **the third generation running to miss exactly one suspect.** |
| `the_tide_waits_for_no_one` | passed, 0 blocking, **0 warnings** | **The glove mechanic working, and working too well.** The cleanest generation to date and the first to score zero coherence warnings: no single-route suspect, every exoneration wired to a witness, no prose leak, narrowings honest. One violation. `E3` narrows to two men — a brandy bottle on a six-foot shelf, naming neither — and `E5` clears the shorter one; intersect and subtract and you have the culprit. That is exactly item 27's design. But **both halves landed on the same witness**, so Dr. Ishbel Marra is a single dealt finding carrying the entire proof and whoever draws her wins without speaking to anyone. Not a regression — the new mechanic meeting the old constraint. The rule it earned: no one finding may reveal both a narrowing and the exoneration that completes it. |
| `snow_on_the_engawa` | passed, 0 blocking, 0 warnings | **Three suspects, against a prompt that has always said exactly four — and nothing checked it.** That is the root cause of its `DEAL.SOLO_SOLVE`, not a separate fault: three suspects means only **two** required exonerations, so any witness revealing both exonerating clues clears everybody. Mori Takeo revealed E1 and E3 and solved the case single-handed; at four suspects the same witness clears 2 of 3 and proves nothing. Now refused by `DEAL.SUSPECT_COUNT`. It also leaked a narrowing in prose for the third generation running, and showed why: `E7` was a vial *"in **Aoyama's** cramped script, found in his entomology kit"*. Once the object **belongs to** somebody the clue leaves possible, naming them is unavoidable — the clue is that person's name with extra steps. A narrowing has to be a class of person, not a possession. |
